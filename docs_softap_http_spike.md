...
4. Tap **Select Y8 BIN**, **Connect to Notua**, and approve Android's exact `NOTUA-...` network request on the first attempt. Confirm the BLE-reported SSID is selected and the UI shows its automatically selected unused candidate slot; if all five slots are active, confirm upload is refused.
5. Tap **Upload**. Keep mobile data enabled. Use the automatically selected candidate from the BLE-reported active-slot list; never hardcode slot 0. Confirm logs show the AP client and that candidate endpoint, progress never stalls, and Flutter reports under 30 seconds and at least 100 KiB/s average.

...
BLE service `7d2a4b70-8e67-4d8b-9f3a-36c89e210001` exposes read-only AP JSON on characteristic `...0007`. Writing ASCII `START_AP` to control characteristic `...0003` starts the AP. HTTP provides `GET /health` and `PUT /images/{slot}`; upload requests require an exact 1,920,000-byte `Content-Length` and hexadecimal CRC32 in `X-Notua-CRC32`.
...
5. Tap **Upload**. Keep mobile data enabled. Confirm logs show the AP client and `/images/0`, progress never stalls, and Flutter reports under 30 seconds and at least 100 KiB/s average.
6. Confirm the structured HTTP response reports 1,920,000 bytes and `crcMatch:true`; confirm firmware reports CRC match/commit and `/images/slot_0.bin` remains valid. A failed/disconnected upload must retain the previous slot file.
7. Trigger the existing playlist/apply and confirm the committed image is displayed on the EPD.
...
Do **not** claim viability until all steps pass on hardware. Automated builds only establish compilation. Viability requires no routing escape, no stall, <30 seconds, >=100 KiB/s, correct CRC/file, EPD display, and network cleanup.
